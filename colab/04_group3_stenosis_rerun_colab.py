"""Colab cells for rerunning Group 3 stenosis metrics after SCT is already installed.

Use this file when:
  - the current Colab runtime already has SCT installed
  - you do NOT want to rerun apt/pip/SCT installation
  - you only want to rerun the Group 3 measurement logic

Expected existing runtime state:
  - `sct_deepseg` and `sct_process_segmentation` are already available
    either on PATH or under `/content/sct/bin`
"""


# =============================================================================
# Colab cell 1/2 - runtime wiring + helper functions
# =============================================================================
...
def _ensure_sct_on_path() -> None:
    if shutil.which("sct_deepseg") and shutil.which("sct_process_segmentation"):
        return
    sct_bin = SCT_DIR / "bin"
    if sct_bin.exists():
        os.environ["PATH"] = f"{sct_bin}:{os.environ['PATH']}"
    if not shutil.which("sct_deepseg") or not shutil.which("sct_process_segmentation"):
        raise RuntimeError(
            "SCT commands are not available in this runtime. "
            "Reopen the runtime where SCT was installed, or rerun the full install notebook."
        )
...
# =============================================================================
# Colab cell 2/2 - run Group 3 stenosis metrics
# =============================================================================

...
def measure_body_slice(level, body_mask_2d, slice_idx):
    ...
    # Implemented as AP span inside a thin SI slab around the body center.
    ...
    return SliceMeasurement(
        ...
        AP_width=dist_mm(points_mm["A_mid"], points_mm["P_mid"]),
...
def average_measurements(level, per_slice):
    ...
    best = valid[len(valid) // 2]  # center slice if present
    out = {
        "level": level,
        "slice_idx": best.slice_idx,
        "AP_width_mm": float(np.mean([m.AP_width for m in valid])),

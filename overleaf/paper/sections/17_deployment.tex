...
segmentation, measurements, assessement, and reporting, each independently containerizable. The
...
assessement layers are CPU-only, so the heavy GPU stage is isolated from the light, frequently changing
measurement logic.
...
per-component status, and the assessed-measurement container (with the four-value status vocabulary),
...
Segmentation is reproducible from versioned notebook workflows (TSS$+$SCT in one environment, SPINEPS in a
separate environment because of the numpy pin). The measurement and validation steps are CPU-only and
...
The project follows a strict session-handoff protocol, granular commits, peer-reviewed integration of
teammate changes, and a hard no-secrets/no-patient-data-in-git rule. Every clinical claim carries a

...
\paragraph{Status.} C1 is the validated \emph{method} (precision $+$ correct lordosis): well-positioned
controls read \SIrange{+15}{+20}{\degree}, matching the standing-radiograph literature. As a
\emph{discriminator}, however, it is \textbf{not} validated: on a representative balanced cohort (26
healthy vs.\ 41 symptomatic) the healthy mean falls to \SI{+10.7}{\degree}, essentially onto the
symptomatic \SI{+8.0}{\degree} ($d=0.28$, AUC $0.57$, $p=0.32$; Section~\ref{sec:results}). The
encouraging $n{=}11$ directional result ($d=0.76$) was a lordosis-biased small sample. Cervical alignment
is biologically too variable (healthy SD $\sim$\SI{10}{\degree}) and supine positioning is a confound, so
we report G4 as a validated \emph{measurement}, not a screen.

...
\sloppy  % prefer slightly loose spacing over text overflowing into the margin
...
backed by committed code, validation results, and the development journal. Companion evidence includes
the final validation verdicts, the consolidated quantitative results, and the full paper manuscript.
...
\begin{itemize}
  ...
  cervical radiology report, threshold-based assessement, or validated cervical \emph{measurements}.
...
measurement-and-assessement system that \emph{uses} segmentation as one input, the way a radiology
...
Measurements IEP   -- G1 vertebral morphometry (Ha/Hp, AP width, tilt)
   ...
   |
   v
Assessement       -- G6 threshold catalog -> status per finding (cited thresholds)
   |
   v
...
G4 C3--C7 Cobb (SPINEPS) & Correct lordosis on well-positioned controls ($+15$--$20^\circ$ $=$
literature); discrimination dissolves on a balanced cohort (26 H vs 41 U: $d=0.28$, AUC 0.57, $p=0.32$) &
Method-valid, NOT a discriminator \\
...
\begin{itemize}
  ...
  \item \textbf{The differentiator is the clinical layer.} Measurement validity, cited assessement,

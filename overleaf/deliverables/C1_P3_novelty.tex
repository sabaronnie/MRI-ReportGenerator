...
validation numbers --- those live in the T1 technical-depth deliverable
(\texttt{T1\_ai\_depth.tex}), the paper (\texttt{overleaf/paper/}), the per-group verdict
(\texttt{docs/validation/group-status-2026-06-08.md}), the consolidated results
(\texttt{docs/validation/results-final-2026-06-08.md}), and the development journal
(\texttt{DEVELOPMENT\_JOURNEY.md}, J1--J26). This deliverable \emph{references} them and argues the
contribution.
...
two cited reasons (full numbers and citations in the positioning document and
\texttt{manual\_baseline\_cost}):
\begin{itemize}
...
\subsection{Cited, auditable interpretation --- never a black box, never a diagnosis}
Every interpreted finding maps to a \emph{cited} threshold in the G6 catalog
(\texttt{services/interpretation/thresholds.py}), with provenance, modality caveats, and an honest label
...
interpretation layer; see T1 for the composition detail) is the defensible choice for a clinical
application:
\begin{itemize}
  ...
  interpretation, and the report --- the parts that decide whether a radiologist trusts the output ---
...
scanner-immunity insight, and a fully cited review-only interpretation layer is the contribution. A
...
\begin{itemize}
  ...
  and a cited review-only interpretation layer.
  ...
  \item Full technical depth: \texttt{T1\_ai\_depth.tex}; methods and numbers: the paper and
  \texttt{results-final-2026-06-08.md}; narrative: \texttt{DEVELOPMENT\_JOURNEY.md}.
\end{itemize}

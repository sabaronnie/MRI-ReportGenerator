"""Tests for the Phase 4 assessement container + catalog-driven status/severity.
...
from services.assessement import (
...
def test_builds_simplified_assessement_container():
    report = {
        "components": {
            "sac": {
                "status": "ok",
    ...
    rows = build_assessed_measurements(
        report,
...
def test_assess_group5_contract_maps_fracture_and_myelomalacia():
    # Group 6 consumes the Group-5 findings contract
    # (services/measurements/group5/flags_contract.py JSON shape) and
    # assesses the compression-screen ratio + the myelomalacia screen via the catalog.
    contract = {
        "levels": [
            {
                "level": "C5",
    ...
    rows = assess_group5_contract(contract)
...
def test_build_reporting_handoff_contract_backfills_case_context_and_syndromes():
    ...
    assert contract["assessements"]["syndromes"] == [
        {

...
- **The fix:** adopt **SCIseg** (`sct_deepseg lesion_sci_t2`, published/validated) as the engine; keep the
  hand-rolled version as an assessable baseline. Our job became the **specificity** check ("on healthy,
...
## J25 — Wiring the pipeline end-to-end: G2 into the orchestrator + demographics into assessement
- **Goal:** make the measurements→assessement→report pipeline actually run end-to-end for a demo, and
...
- **G2 wired:** added the four disc components (`disc_si_height`, `disc_height_index`, `disc_ap_bulge`,
  ...
  flow through and `classify()` assesses them (their output keys already match the catalog).
- **Demographics wired:** `load_context`/`run_all` now carry `age/sex/height`; the report has a `patient`
  block; `build_assessed_measurements` records `demographics_used` per measurement and passes `sex` to
...
- **Correction to my own earlier claim:** `build_assessed_measurements` was NOT a pure scaffold — it
